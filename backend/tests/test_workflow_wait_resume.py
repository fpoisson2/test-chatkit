...
def test_wait_in_while_keeps_pending_input_when_payload_has_no_source(monkeypatch):
    async def _run() -> None:
        WorkflowInput = executor.WorkflowInput
        run_workflow = executor.run_workflow
        ...
        # Retry without a source_item_id but with a reconstructed user message
        resumed_message = SimpleNamespace(
            id="msg-2",
            thread_id=agent_context.thread.id,
            created_at=datetime.now(),
        ...
        retry_input = WorkflowInput(input_as_text="Bonjour", source_item_id=None)
        summary_two = await run_workflow(
            retry_input,
            agent_context=agent_context,
            workflow_definition=definition,
        ...
        wait_state_after_retry = agent_context.thread.metadata.get(_WAIT_STATE_METADATA_KEY)
        assert wait_state_after_retry is not None
        # The wait should still point to the original input id since no new user message was provided
        assert wait_state_after_retry.get("input_item_id") == "msg-1"
