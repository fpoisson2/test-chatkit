...
class ComputerUseNodeHandler(BaseNodeHandler):
    ...
    async def _initialize_environment(
        ...
        from chatkit.agents import set_current_computer_tool
        ...
        # Ensure this tool is set as the current computer tool for the agent context
        # This allows stream_agent_response to find the tool and register debug sessions
        # for agent-generated tasks.
        set_current_computer_tool(computer_tool)
    ...
    async def execute(
        ...
        if computer_use_config:
            ...
            if tokens:
                # Only emit the task manually if we are NOT in agent mode.
                # In agent mode, the agent will emit the task when it uses the tool.
                # Initializing the environment (above) is sufficient to register the tool/token.
                if mode != "agent":
