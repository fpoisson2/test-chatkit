"""Orchestration du workflow ChatKit et définition des agents associés."""
...
from agents import Agent, RunConfig, RunContextWrapper, Runner, TResponseInputItem
...
from .schemas import RDacteurSchema, Triage2Schema, TriageSchema
from .tools import (
...
def _build_agent_kwargs(
    ...
    if "model_settings" in merged:
        merged["model_settings"] = coerce_model_settings(merged["model_settings"])
    if "tools" in merged:
        merged["tools"] = coerce_agent_tools(
    ...
    if "response_format" in merged:
        ...
        if sync_output_type:
            ...
            resolved = build_output_type_from_response_format(
                response_format,
    ...
    *,
    ...
    for step in agent_steps_ordered:
        ...
        if widget_config is not None and widget_config.output_model is not None:
            ...
            try:
                overrides["response_format"] = create_response_format_from_pydantic(
    ...
    async def record_step(step_key: str, title: str, payload: Any) -> None:
        formatted_output = format_step_output(payload)
        print(
        ...
        if current_node.kind == "watch":
            ...
            if on_stream_event is not None:
                if payload_to_display is None:
                    ...
                    formatted_payload = format_step_output(payload_to_display)
