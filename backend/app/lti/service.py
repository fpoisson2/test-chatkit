...
def _extract_platform_subject(payload: Mapping[str, Any]) -> str | None:
    """Return the platform subject (``sub`` claim) as a string if available."""
...
class LTIService:
    ...
    def _get_deployment(
        ...
        if deployment is None:
            # Auto-create deployment on first use
            logger.info(
                "LTI deployment not found for deployment_id=%r (registration_id=%s). "
                "Auto-creating new deployment.",
                deployment_id,
                registration.id,
            )
            ...
            self.session.flush()
            logger.info(
                "LTI deployment auto-created: id=%s, deployment_id=%r, registration_id=%s",
                deployment.id,
