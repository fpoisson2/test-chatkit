...
class LTIAGSClient(AGSClientProtocol):
    ...
    @staticmethod
    def _normalize_score(score: float | None, score_maximum: float | None) -> float:
        """Normalize a raw score to the [0, 1] interval expected by Moodle."""
        ...
        try:
            maximum = float(score_maximum) if score_maximum is not None else None
        except (TypeError, ValueError):
            maximum = None
