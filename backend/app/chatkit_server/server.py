    ...
    async def _run_loop(self) -> None:
        ...
        try:
            while True:
                ...
                # logger.debug("StreamProcessor received event: %s", type(event))
                ...
                if isinstance(event, ThreadItemAddedEvent):
                    ...
                    # Persist immediately if no listeners (background mode)
                    if not self.listeners and self._context:
                        try:
                            logger.debug("Background persistence: Adding item %s", event.item.id)
                        ...
                        if isinstance(
                            ...
                            if not self.listeners:
                                ...
                                if (
                                    ...
                                    try:
                                        logger.debug("Background persistence: Saving item update %s (text delta)", event.item_id)
                            ...
                            if not self.listeners:
                                ...
                                if (
                                    ...
                                    try:
                                        logger.debug("Background persistence: Saving item update %s (workflow task)", event.item_id)
                                        await self.store.save_item(
                                            self.thread.id, item, context=self._context
                                        )
                                        last_save_time[event.item_id] = now
                    ...
                    if not self.listeners and self._context:
                        try:
                            ...
                            # Use add_thread_item which is an upsert for PostgresChatKitStore
                            logger.debug("Background persistence: Final save for item %s", item.id)
                            await self.store.add_thread_item(
                                self.thread.id, item, context=self._context
                            )
                        except Exception:
