...
def test_resume_streaming_after_cancellation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def _run() -> None:
        root_dir = Path(__file__).resolve().parents[3]
        ...
        async def _fake_execute_workflow(
            ...
            *,
            ...
            third_item = AssistantMessageItem(
                id=self.store.generate_item_id(
                    "message", thread, agent_context.request_context
                ),
                thread_id=thread.id,
                created_at=datetime.now(),
                content=[AssistantMessageContent(text="Dernier message")],
            )
            await event_queue.put(ThreadItemDoneEvent(item=third_item))
        ...
        await server.store.add_thread_item(thread.id, user_message, context)
        ...
        async def _consume_stream() -> None:
            ...
            try:
                async for event in server._process_events(
                    ...
                    consumed.append(event)
                    if len(consumed) == 1:
                        # Récupérer l'ID du premier item
                        if isinstance(event, ThreadItemDoneEvent):
                            first_item_id = event.item.id
                        first_event_received.set()
                    await asyncio.sleep(0)
            except asyncio.CancelledError:
                raise
        ...
        # Attendre que les événements soient bufferisés
        for _ in range(50):
            await asyncio.sleep(0.1)

        # Reprendre le streaming depuis le premier item
        resumed_events: list[ThreadStreamEvent] = []
        async for event in server.resume_streaming(thread.id, first_item_id):
            resumed_events.append(event)

        # Vérifier qu'on a bien récupéré les événements après le premier
        resumed_texts = [
            content.text
        ...
        assert "Dernier message" in resumed_texts
        assert "Avant annulation" not in resumed_texts  # Ne devrait pas être inclus
