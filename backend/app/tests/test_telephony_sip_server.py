...
async def test_invite_configures_rtp_remote_target(
    ...
    class _FakeRtpServer:
        ...
        async def send_audio(self, _: bytes) -> None:
            recorded.setdefault("send_audio_called", 0)

        async def packet_stream(self):
            if False:  # pragma: no cover - générateur factice
                yield b""
...
@pytest.mark.anyio
@pytest.mark.parametrize("anyio_backend", ["asyncio"])
async def test_invite_remote_target_from_handler_result(
    monkeypatch: pytest.MonkeyPatch, anyio_backend: str
) -> None:
    del anyio_backend
    ...
    class _FakeRtpServer:
        ...
        def set_remote_target(self, host: str | None, port: int | None) -> None:
            recorded.setdefault("remote_calls", []).append((host, port))
    ...
    monkeypatch.setattr(
        startup_module,
        "_extract_remote_media_target",
        lambda _: (None, None),
    )

    async def _fake_handle_incoming_invite(*_: Any, **__: Any) -> InviteHandlingResult:
        return InviteHandlingResult(
            remote_host="198.51.100.9",
            remote_port=49170,
            selected_codec=SelectedCodec(payload_type=0, name="pcmu", clock_rate=8000),
        )
    ...
    remote_calls = recorded.get("remote_calls", [])
    assert remote_calls[-1] == ("198.51.100.9", 49170)
