                    ...
                    if isinstance(event, RealtimeAudio):
                        ...
                        if not block_audio_send_ref[0]:
                            if pcm_data:
                                ...
                                # Envoyer directement l'audio @ 24kHz à send_to_peer
                                # send_to_peer va maintenant:
                                # 1. Rééchantillonner 24kHz → 8kHz dans un thread séparé (asyncio.to_thread)
                                # 2. Filtrer silence (si max_amp <= 4)
                                # 3. Découper en frames de 320B @ 8kHz
                                # 4. Hysteresis backpressure (HW/LW)
                                # 5. Enqueuer (pas de boucle d'attente)
