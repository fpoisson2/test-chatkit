...
class AudioMediaPort(pj.AudioMediaPort if PJSUA_AVAILABLE else object):
    ...
    # Compteur de ports créés pour avoir des noms uniques
    _port_counter = 0

    def __init__(self, adapter: PJSUAAdapter, port_name: str | None = None):
        if not PJSUA_AVAILABLE:
            return
        ...
        # Créer la configuration du port avec un nom unique
        ...
        # Utiliser un nom unique pour éviter les conflits dans le conference bridge
        if port_name is None:
            AudioMediaPort._port_counter += 1
            port_name = f"chatkit_audio_{AudioMediaPort._port_counter}"
        self._port_name = port_name

        self.createPort(port_name, port_info)
        logger.info(f"✅ Port audio créé avec le nom: {port_name}")
        ...
        if ci.media:
            ...
            for mi in ci.media:
                if mi.type == pj.PJMEDIA_TYPE_AUDIO and mi.status == pj.PJSUA_CALL_MEDIA_ACTIVE:
                    ...
                    # SOLUTION FINALE: Détruire complètement l'ancien port global AVANT de créer le nouveau
                    # PJSUA réutilise toujours le slot 2 pour les ports customs, donc on doit:
                    # 1. Détruire l'ancien port global s'il existe
...
class PJSUAAdapter:
    ...
    def _do_teardown_call_resources(self, call: PJSUACall, call_id: int) -> None:
        ...
        if audio_port:
            ...
            try:
                ...
                if hasattr(audio_port, '__del__'):
                    try:
                        audio_port.__del__()
                        logger.debug("✅ __del__() appelé sur le port")
                    except Exception as e:
                        logger.debug("Erreur __del__: %s", e)

                # Supprimer toutes les références Python pour forcer le GC
                del audio_port
                logger.info("✅ Port audio détruit et déréférencé (call_id=%s)", call_id)
        ...
        try:
            import gc
            gc.collect()
            logger.info("🧹 Garbage collection forcé (call_id=%s)", call_id)
        except Exception as e:
    ...
    def _dispose_call_object(self, call: PJSUACall, call_id: int) -> None:
        ...
        if getattr(call, "_disposed", False):
            return

        delete_fn = getattr(call, "delete", None)
        if callable(delete_fn):
            try:
                delete_fn()
                logger.debug("🗑️  Objet PJSUA Call détruit (call_id=%s)", call_id)
            except Exception as e:
                logger.warning("Impossible de détruire l'objet Call (call_id=%s): %s", call_id, e)
    ...
    async def _on_call_state(self, call: PJSUACall, call_info: Any) -> None:
        ...
        if call_info.state == pj.PJSIP_INV_STATE_DISCONNECTED:
            ...
            # Retirer l'appel des actifs pour éviter toute utilisation ultérieure
            self._active_calls.pop(call_info.id, None)

            # Libérer immédiatement les ressources audio associées
            self._teardown_call_resources(call, call_info.id)
    ...
    async def hangup_call(self, call: PJSUACall) -> None:
        ...
        if getattr(call, "_disconnected", False) or getattr(call, "_disposed", False):
            logger.debug("Appel déjà terminé ou détruit, hangup ignoré")
            return
    ...
    async def cleanup_call(self, call_id: int) -> None:
        ...
        try:
            ...
            self._teardown_call_resources(call, call_id)

            if getattr(call, "_disconnected", False):
                # L'appel est déjà terminé, détruire l'objet immédiatement
                self._dispose_call_object(call, call_id)
            ...
            if call and self._is_call_valid(call):
                ...
                logger.debug("Appel %s déjà invalide ou terminé, destruction directe", call_id)
                self._dispose_call_object(call, call_id)

            # Retirer de active_calls pour éviter toute fuite de référence
            self._active_calls.pop(call_id, None)
            logger.info("✅ Nettoyage terminé (call_id=%s)", call_id)
    ...
    def _is_call_valid(self, call: PJSUACall) -> bool:
        ...
        if (
            not PJSUA_AVAILABLE
            or not call
            or getattr(call, "_disconnected", False)
            or getattr(call, "_disposed", False)
        ):
            return False
    ...
    def send_audio_to_call(self, call: PJSUACall, audio_data: bytes) -> None:
        ...
        if getattr(call, "_disconnected", False) or getattr(call, "_disposed", False):
            logger.debug("Tentative d'envoi audio sur un appel terminé, ignore")
            return
