...
4. **Optionnel : contournez la vérification distante** — définissez `VITE_CHATKIT_SKIP_DOMAIN_VERIFICATION=true` si votre environnement ne peut pas joindre l'API OpenAI `domain_keys/verify_hosted`. Dans ce cas, toute requête de vérification est court‑circuitée côté navigateur et le widget continue de fonctionner. Lorsque cette variable est absente mais que votre serveur ne répond pas sur `domain_keys/verify_hosted` (statut `404`, `405`, `501` ou absence de réseau), `MyChat` neutralise désormais automatiquement l'appel afin d'éviter la disparition du widget.
