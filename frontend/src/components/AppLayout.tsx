...
function LabSidebarMenu({ token, tabIndex, collapsed }: { token: string; tabIndex: number; collapsed: boolean }) {
  ...
  return <nav className={`chatkit-sidebar__lab-menu${collapsed ? " chatkit-sidebar__lab-menu--compact" : ""}`} aria-label="Laboratoires"><div className="chatkit-sidebar__lab-menu-heading"><span>Laboratoires</span><button type="button" onClick={() => navigate("/lab-review?new=1")} tabIndex={tabIndex} aria-label="Nouveau laboratoire">+</button></div>{labs.map(lab => <button type="button" key={lab.slug} onClick={() => navigate(`/lab-review?lab=${encodeURIComponent(lab.slug)}`)} tabIndex={tabIndex} className="chatkit-sidebar__lab-item" aria-label={lab.title}>{collapsed ? "L" : lab.title}</button>)}</nav>;
}
