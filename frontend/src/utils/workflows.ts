...
export const getStartTelephonyEntryPoint = (
  parameters: AgentParameters | null | undefined,
): boolean => {
  if (!parameters) {
    return false;
  ...
  return telephony.sip_entrypoint === true;
};

export const setStartTelephonyEntryPoint = (
  parameters: AgentParameters,
  enabled: boolean,
): AgentParameters =>
  updateStartTelephonyConfig(parameters, (current) => {
    if (!enabled) {
      const { sip_entrypoint: _ignored, ...rest } = current;
      return rest;
    }

    return { ...current, sip_entrypoint: true };
  });
...
export const resolveStartParameters = (
  ...
  result = setStartTelephonyEntryPoint(
    result,
    getStartTelephonyEntryPoint(rawParameters),
  );
