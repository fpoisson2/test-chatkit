...
type McpServerModalProps = {
  ...
  onTestConnection: (config: LegacyMcpSseToolConfig) => Promise<McpTestResult>;
};
  ...
  const handleTestConnection = async () => {
    ...
    try {
      const result = await onTestConnection({
        url,
        authorization: formState.authorization.trim(),
      });
...
export const ToolSettingsPanel = ({
  ...
  const mcpConfig = getLegacyMcpSseConfig(parameters);
  ...
  const modal = isCreateModalOpen ? (
    <McpServerModal
      ...
      onTestConnection={onTestMcpSseConnection}
    />
  ) : null;
        ...
        <div className={styles.nodeInspectorPanelInner}>
          ...
          {mcpEnabled ? (
            <>
              ...
              {mcpTestState.status === "success" &&
              mcpTestState.toolNames.length > 0 ? (
                <div className={styles.nodeInspectorInfoMessage}>
                  {t(
                    "workflowBuilder.agentInspector.mcpTestStatus.toolListLabel",
                  )}
                  : {mcpTestState.toolNames.join(", ")}
                </div>
              ) : null}
            </>
          ) : (
