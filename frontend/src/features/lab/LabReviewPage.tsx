...
type Attempt = { id: string; status: string; revision: number; updated_at: string; version: number; answers: Record<string, unknown>; field_grades: Record<string, { rating: "correct" | "partial" | "incorrect" | "ungraded"; comment?: string }>; response_fields: { id: string; label: string; type: string; unit?: string; points?: number; rows?: { id: string; label: string }[]; columns?: { id: string; label: string; unit?: string; input_type?: string }[]; visible_columns?: string[] }[]; teacher_validations: Record<string, Validation>; teacher_validation_fields: { id: string; label: string; section?: string }[]; user: { email: string; display_name?: string }; score?: number; feedback?: string };
...
export default function LabReviewPage() {
  ...
  const loadLab = useCallback(async () => {
    ...
    const next = await a.json() as Attempt[]; setAttempts(next); setVersions(await v.json() as Version[]); const latest = [...next].sort((left, right) => right.version - left.version || Date.parse(right.updated_at) - Date.parse(left.updated_at))[0]; setSelectedId((current) => next.some((item) => item.id === current) ? current : latest?.id); setLastRefresh(new Date());
  }, [token, slug]);
  ...
  const generateFeedback = async () => { if (!selected || generating) return; setGenerating(true); setMessage("Génération de la rétroaction…"); try { const response = await fetch(`/api/labs/admin/attempts/${selected.id}/generate-feedback`, { method: "POST", headers }); if (!response.ok) { setError((await response.json()).detail ?? "Génération impossible"); setMessage(""); return; } const result = await response.json(); setFeedback(result.feedback); setMessage("Rétroaction générée avec gpt-5.6-luna. Relisez-la avant de l’enregistrer."); } finally { setGenerating(false); } };

  return <main className="lab-shell"><header className="lab-topbar"><h1>{currentLab?.title ?? slug}</h1>{slug && <div className="lab-admin-actions"><button className="lab-button lab-button--primary" onClick={() => setEditing(true)}><Edit3 size={17} /> Modifier</button></div>}</header>
    ...
    {slug && <AttemptSelectors attempts={attempts} versions={versions} selectedId={selectedId} onSelect={setSelectedId} />}
    {selected && <LabResponseReview attempt={selected} onGrade={gradeField} validations={selected.teacher_validation_fields.map(field => ({ ...field, ...selected.teacher_validations[field.id] }))} onValidate={(fieldId, approved) => void action(`/api/labs/admin/attempts/${selected.id}/teacher-validation`, { field_id: fieldId, approved, comment: approved ? "Étape vérifiée" : "Validation retirée" })} />}
    {selected && <FeedbackEditor value={feedback} onChange={setFeedback} onGenerate={generateFeedback} busy={generating} />}
    {selected && <footer className="lab-review-footer"><button className="lab-button lab-button--primary" onClick={() => action(`/api/labs/admin/attempts/${selected.id}/grade`, { score: Number(score), maximum: 100, feedback, publish_to_moodle: false })}><ClipboardCheck size={17} /> Enregistrer l’évaluation</button><button className="lab-button" onClick={() => action(`/api/labs/admin/attempts/${selected.id}/grade`, { score: Number(score), maximum: 100, feedback, publish_to_moodle: true })}><Send size={17} /> Publier dans Moodle</button></footer>}
...
function AttemptSelectors({ attempts, versions, selectedId, onSelect }: { attempts: Attempt[]; versions: Version[]; selectedId?: string; onSelect: (id: string) => void }) {
  const selected = attempts.find(attempt => attempt.id === selectedId);
  const students = Array.from(new Map(attempts.map(attempt => [attempt.user.email, attempt.user])).values());
  const studentAttempts = attempts.filter(attempt => attempt.user.email === selected?.user.email).sort((left, right) => right.version - left.version || Date.parse(right.updated_at) - Date.parse(left.updated_at));
  const chooseStudent = (email: string) => onSelect(attempts.filter(attempt => attempt.user.email === email).sort((left, right) => right.version - left.version || Date.parse(right.updated_at) - Date.parse(left.updated_at))[0]?.id ?? "");
  return <div className="lab-attempt-selectors"><label>Étudiant<select value={selected?.user.email ?? ""} onChange={event => chooseStudent(event.target.value)}><option value="">Sélectionner un étudiant</option>{students.map(student => <option key={student.email} value={student.email}>{student.display_name || student.email}</option>)}</select></label><label>Version<select value={selected?.id ?? ""} onChange={event => onSelect(event.target.value)} disabled={!studentAttempts.length}><option value={studentAttempts[0]?.id ?? ""}>Dernière version</option>{versions.filter(version => studentAttempts.some(attempt => attempt.version === version.version)).map(version => { const attempt = studentAttempts.find(item => item.version === version.version); return attempt && <option key={attempt.id} value={attempt.id}>Version {version.version}</option>; })}</select></label></div>;
}
