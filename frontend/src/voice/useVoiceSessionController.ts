...
type StartOptions = {
  ...
  secretOverride?: VoiceSessionSecret | null;
};
...
export type VoiceSessionControllerOptions = {
  fetchSecret?: (() => Promise<VoiceSessionSecret>) | null;
};

export type UseVoiceSessionControllerResult = {
...
export const useVoiceSessionController = (
...
): UseVoiceSessionControllerResult => {
  const fetchSecretRef = useRef<(() => Promise<VoiceSessionSecret>) | null>(options.fetchSecret ?? null);
  const latestSecretRef = useRef<VoiceSessionSecret | null>(null);
  ...
  useEffect(() => {
    fetchSecretRef.current = options.fetchSecret ?? null;
  }, [options.fetchSecret]);
  ...
  const handleRefreshDue = useCallback(() => {
    ...
    const secret = latestSecretRef.current;
    if (fetchSecretRef.current) {
      void start({ preserveHistory: true });
      return;
    }
    if (secret) {
      void start({ preserveHistory: true, secretOverride: secret });
    }
  }, []);
  ...
  const startSession = useCallback(
    async ({ preserveHistory = false, secretOverride = null }: StartOptions = {}) => {
      ...
      try {
        const secret =
          secretOverride ??
          (fetchSecretRef.current ? await fetchSecretRef.current() : latestSecretRef.current);
        if (!secret) {
          throw new Error("Secret temps réel indisponible pour démarrer la session vocale.");
        }
    ...
    [addError, clearErrors, connect, disconnect, resetTranscripts, status],
  );
  ...
  const value = useMemo<UseVoiceSessionControllerResult>(
    () => ({
      status,
      isListening,
      transcripts,
...
export type { StartOptions as VoiceSessionStartOptions, StopOptions as VoiceSessionStopOptions };
