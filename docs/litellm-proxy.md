...
2. Définissez les variables spécifiques au proxy :
   ...
   ```
3. Lancez uniquement le proxy pour vérifier qu'il démarre correctement (l'image publique [Docker Hub `litellm/litellm:main-latest`](https://hub.docker.com/r/litellm/litellm) est utilisée dans `docker-compose.yml`) :
   ```bash
