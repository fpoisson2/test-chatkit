...
1. Ajoutez vos clés d'API fournisseurs dans votre `.env` (ex. `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `MISTRAL_API_KEY`). Pour les
   ...
   le proxy et exporte automatiquement `LITELLM_DATABASE_URL` (ainsi que la variable `DATABASE_URL` exigée par Prisma) avec cette valeur, ce qui évite de partager un DSN SQLAlchemy (`postgresql+psycopg://…`)
   que LiteLLM/Prisma ne sait pas interpréter. Ajustez cette valeur si vous déployez LiteLLM sur une autre base ou si vous changez les identifiants du service `litellm-db`.
